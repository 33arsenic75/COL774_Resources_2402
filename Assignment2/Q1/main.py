...
def word_cloud(df_train, df_test, smoothening, tokenizer, qnum = "1_1b", column_name = ["Description"]):
    ...
    model = NaiveBayes()
    model.fit(df_train, smoothening=smoothening
                , class_col="Class Index", text_col=f"Tokenized {column_name}")
...
def compare_methods(df_train, df_test, column_name ,smoothening=1):
    results = []
...
def q1_7(df_train, df_test, smoothening):
...
# def q1_9(df_train, df_test, smoothening):
